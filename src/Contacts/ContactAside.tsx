...
import { Box, Typography, Divider, Link, Button, Chip, Menu, Dialog, DialogActions, DialogContent, MenuItem, DialogTitle, TextField } from '@mui/material';
...
export const ContactAside = ({
    ...
    const unselectedTagIds: Tag[] = allTags.filter(t => !contactTags.find(ct => ct.tag.tag === t.tag));
    useEffect(() => {
    ...
    const handleCreateTag = async (event: FormEvent<HTMLFormElement>) => {
        ...
        setIsSaving(true);
        try {
            const newTag = await tagsRepo.insert({ tag: newTagName, color: newTagColor });
            ...
            <Box ml={4} width={250} minWidth={250}>
                ...
                >
                    <form onSubmit={handleCreateTag}>
                        ...
                        <DialogActions>
                            <Button onClick={() => setCreateTagDialogOpen(false)} color="primary">
                                Cancel
                            </Button>
                            <Button
                                type="submit"
...
const RoundButton = ({ color, handleClick, selected }: { color: string, selected: boolean, handleClick: () => void }) => {
